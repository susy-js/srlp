...
/**
 * SRLP Encoding based on: https://octonion.institute/susy-go/wiki/wiki/%5BEnglish%5D-SRLP
...
function safeParseInt(v: string, base: number): number {
  if (v.slice(0, 2) === '00') {
    throw new Error('invalid SRLP: extra zeros')
  }
...
/**
 * SRLP Decoding based on: {@link https://octonion.institute/susy-go/wiki/wiki/%5BEnglish%5D-SRLP|SRLP}
 * @param input - will be converted to buffer
...
/**
 * Get the length of the SRLP input
 * @param input
...
function _decode(input: Buffer): Decoded {
  ...
  if (firstByte <= 0x7f) {
    // a single byte whose value is in the [0x00, 0x7f] range, that byte is its own SRLP encoding.
    return {
    ...
    if (length === 2 && data[0] < 0x80) {
      throw new Error('invalid srlp encoding: byte must be less 0x80')
    }
    ...
    if (totalLength > input.length) {
      throw new Error('invalid srlp: total length is larger than the data')
    }

    innerRemainder = input.slice(llength, totalLength)
    if (innerRemainder.length === 0) {
      throw new Error('invalid srlp, List has a invalid length')
    }
